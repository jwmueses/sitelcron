...
public class FacturaElectronica extends Xml{
            ...
            String pIvas, String codigoIvas, String direccion, String plan)
    {
        ...
        Element infoFactura = this.getElementoActivo();
            ...
            this.nuevoElementoInsertar("totalSinImpuestos", this.truncar(Float.parseFloat(totalSinImpuestos) - Float.parseFloat(totalDescuento)  ) );
            this.nuevoElementoInsertar("totalDescuento", this.truncar(totalDescuento) );
            ...
            this.nuevoElemento(infoFactura, "totalConImpuestos", false);
                this.nuevoElementoInsertar("totalImpuesto");
                    ...
                    if(Float.parseFloat(subtotal_0)>0 ){
                        ...
                        this.nuevoElementoInsertar("baseImponible", this.truncar(Float.parseFloat(subtotal_0) - Float.parseFloat(totalDescuento) ) );
                    ...
                    if(Float.parseFloat(subtotal_3)>0 ){
                        ...
                        this.nuevoElementoInsertar("baseImponible", this.truncar(Float.parseFloat(subtotal_3) - Float.parseFloat(totalDescuento) ) );
                        this.nuevoElementoInsertar("valor", this.truncar(iva_3) );
                    }
            
        this.setElementoActivo(infoFactura);
            this.nuevoElementoInsertar("propina", "0.00");
            this.nuevoElementoInsertar("importeTotal", this.truncar(importeTotal) );
        ...
        for(int i=0; i<vec_codigo.length;i++){
            ...
            this.nuevoElementoInsertar("cantidad", this.truncar(vec_cantidad[i]) );
            this.nuevoElementoInsertar("precioUnitario", String.valueOf ( this.redondear(Double.parseDouble(vec_precioUnitario[i]), 4) ) );
            this.nuevoElementoInsertar("descuento", this.truncar(vec_descuento[i]) );
            this.nuevoElementoInsertar("precioTotalSinImpuesto", this.truncar(Float.parseFloat(vec_subtotal[i]) - Float.parseFloat(vec_descuento[i]) ) );
            this.nuevoElementoInsertar("impuestos");
                this.nuevoElementoInsertar("impuesto");
                    ...
                    this.nuevoElementoInsertar("baseImponible", this.truncar(Float.parseFloat(vec_subtotal[i]) - Float.parseFloat(vec_descuento[i])));
                    this.nuevoElementoInsertar("valor", this.truncar(vec_iva[i]) );
        }
    ...
    public String truncar(String num2)
    {
