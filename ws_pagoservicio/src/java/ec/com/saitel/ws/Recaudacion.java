...
public class Recaudacion {
    ...
    private String codigoError = "01";
    private String mensaje = "No se ha proporcionado una clave de acceso";
                                    ...
                                    if(objDB.getFilas(rs)>0){
                                        ...
                                        while(rs.next()){
                                            ...
                                            if(id_instalacion.compareTo(aux)!=0){
                                                ...
                                                xml.append("<subtotal>");
                                                xml.append(rs.getString("subtotal")!=null ? rs.getString("subtotal") : "0.00");
                                                xml.append("</subtotal>");
                                                
                                                xml.append("<subtotal_0>");
                                                xml.append(rs.getString("subtotal_0")!=null ? rs.getString("subtotal_0") : "0.00");
                                                xml.append("</subtotal_0>");
                                                
                                                xml.append("<subtotal_12>");
                                                xml.append(rs.getString("subtotal_2")!=null ? rs.getString("subtotal_2") : "0.00");
                                                xml.append("</subtotal_12>");
                                                
                                                xml.append("<iva_12>");
                                                xml.append(rs.getString("iva_2")!=null ? rs.getString("iva_2") : "0.00");
                                                xml.append("</iva_12>");
...
//                                    }
                        ...
                        objDB.ejecutar("INSERT INTO tbl_auditoria(alias,ip_maquina,hora,fecha,transaccion) " +
                            ...
                            "', 'CONSULTA DE PREFACTURA(S) A TRAVES DEL WEB SERVICES PAGO EN LINEA PARA EL CLIENTE CON NUMERO DE IDENTIFICACION "+dni+"');");
                    }catch(Exception e){
                        codigoError = "05";
                         mensaje = "Error 500";
                    }
...
//                if( (puntoEmision[1].compareTo("t")==0 && saldo>0) || puntoEmision[1].compareTo("f")==0 ){      //  valida si es prepago
    ...
    private String prefacturaEmitir(String idPrefactura, String num_documento, String totalCobrado, String idPuntoEmision)
    {
        ...
        if ( codigoError.compareTo("15")!=0 && codigoError.compareTo("16")!=0 ) {
...
//            int actualizarMikrotiksEnLinea = -1;
    ...
    //        double  p_iva = Integer.parseInt( objDB.getValor("p_iva1") );
            ...
            try{
                ...
                ResultSet rsBanco = objDB.consulta("SELECT * FROM vta_banco where lower(banco) like '%produbanco%'");
                if(rsBanco.next()){
