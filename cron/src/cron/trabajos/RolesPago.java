...
public class RolesPago implements Job{
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        System.out.println(Fecha.getFecha("SQL") + " " + Fecha.getHora() + ": Inicio de proceso de generacion de roles de pago.");
        
        try {
            
            boolean generar = true;
            try{
                ResultSet rs = objDataBase.consulta("select date_trunc( 'month', max(fecha_inicial) )::date <> date_trunc( 'month', now() )::date from tbl_rol_pagos");
                if( rs.next() ) {
                    generar = rs.getString(1)!=null ? rs.getBoolean(1) : true;
                    rs.close();
                }
            }
            catch(Exception e){
        ...
        //                       cobraReserva=""+((comision+extra+suple+Double.parseDouble(sumDeducible)+Double.parseDouble(sueldo))/12);
                ...
                System.out.println("Roles de pago ya se encuentran generados para el período actual");
                
            }
            
        }finally{
            System.out.println(Fecha.getFecha("SQL") + " " + Fecha.getHora() + ": Finalizando de proceso de generacion de roles de pago");
            objDataBase.cerrar();
        }
