 ...
 * @author wilso
 */
public class FrmCambiarClaveGuardar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String id_cliente_portal = request.getParameter("id_cliente_portal");
        String clave_anterior = request.getParameter("clave_anterior");
        String clave_nueva = request.getParameter("clave_nueva");
        String clave_nueva1 = request.getParameter("clave_nueva1");
        ...
        try {
            ...
            boolean ok = ObjUsuario.esValidaClaveAnterior(id_cliente_portal, clave_anterior);
            if (ok) {
                ok = ObjUsuario.setCambioClave(id_cliente_portal, clave_nueva);
                if (ok) {
                    sesion.setAttribute("clave_temporal", false);
                    ObjAuditoria.setRegistro(request, "PORTAL WEB SE REALIZO CAMBIO DE CLAVE AL ID PORTAL : " + id_cliente_portal);
                    r = "err»0^vta»vta_clave^fun»window.open('Salir','_self');^msg»Actualización de la contraseña registrada satisfactoriamente";
                }
            } else {
                r = "msg»La contraseña anterior ingresada es invalida";
            }
            out.print(r);
        } catch (Exception e) {
