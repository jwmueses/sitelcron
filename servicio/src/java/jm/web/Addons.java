...
public class Addons {
    ...
    public static String redondearDecimales(double num, int decimales) {
        double parteEntera, resultado;
        resultado = num;
        parteEntera = Math.floor(resultado);
        resultado = (resultado - parteEntera) * Math.pow(10, decimales);
        resultado = Math.round(resultado);
        resultado = (resultado / Math.pow(10, decimales)) + parteEntera;
    ...
    public static Image setMarcaAgua(byte[] logo, int ancho, int alto) {
        try {
            Image imagelogo = Image.getInstance(logo);
            imagelogo.setTransparency(new int[]{255, 255});
            imagelogo.setRotationDegrees(57);
