...
public class DatosDinamicos {
     ...
     */
    public static String combo(String nom, String mar, String[][] opt, String onC) {
        String sel = "";
        StringBuilder cad = new StringBuilder();
        cad.append("<select id=\"" + nom + "\" name=\"" + nom + "\" class=\"form-control\" onchange=\"" + onC + "\">");
        for (int i = 0; i < opt.length; i++) {
            sel = (opt[i][0].compareTo(mar) == 0) ? "selected" : "";
    ...
    public static String comboRequired(String nom, String mar, String[][] opt, String onC) {
        String sel = "";
        StringBuilder cad = new StringBuilder();
        cad.append("<select id=\"" + nom + "\" name=\"" + nom + "\" class=\"form-control\" onchange=\"" + onC + "\" required>");
        for (int i = 0; i < opt.length; i++) {
            sel = (opt[i][0].compareTo(mar) == 0) ? "selected" : "";
            cad.append("<option " + sel + " value=\"" + opt[i][0] + "\">" + opt[i][1] + "</option>");
        }
    ...
    public static String combo(String nom, String mar, String[][] opt, String onC, int ancho) {
        String sel = "";
        StringBuilder cad = new StringBuilder();
    ...
    public static String comboDisabled(String nom, String mar, String[][] opt, String onC, String estado) {
        String sel = "";
        StringBuilder cad = new StringBuilder();
        cad.append("<select id=\"" + nom + "\" name=\"" + nom + "\" " + estado + " class='form-control' onchange=\"" + onC + "\">");
        for (int i = 0; i < opt.length; i++) {
            sel = (opt[i][0].compareTo(mar) == 0) ? "selected" : "";
            cad.append("<option " + sel + " value=\"" + opt[i][0] + "\">" + opt[i][1] + "</option>");
        }
     ...
     * @return una cadena con la etiqueta SELECT.
     */
    public static String combo(String nom, String mar, String[][] opt, String onC, String opc) {
        String sel = "";
        StringBuilder cad = new StringBuilder();
    ...
    public static String combogroup(ResultSet dat, String nom, String mar, String onC) {
        String sel = "";
        StringBuilder cad = new StringBuilder();
        cad.append("<select id=\"" + nom + "\" name=\"" + nom + "\" onchange=\"" + onC + "\">");
        try {
    ...
    public static String combogroup2(ResultSet dat, String nom, String mar, String onC) {
        String sel = "";
        StringBuilder cad = new StringBuilder();
        cad.append("<optgroup label='No Disponibles'>");
        try {
            ResultSetMetaData mdata = dat.getMetaData();
            int lim = mdata.getColumnCount();
            String et = "";
            ...
            while (dat.next()) {
                ...
                cad.append("<option " + sel + " value=\"" + dat.getString(1) + "\" disabled>" + et + "</option>");
                et = "";
            }
            dat.beforeFirst();
        } catch (Exception e) {
    ...
    public static String combo(ResultSet dat, String nom, String mar, String onC, int anch) {
        String sel = "";
        StringBuilder cad = new StringBuilder();
        cad.append("<select id=\"" + nom + "\" name=\"" + nom + "\" onchange=\"" + onC + "\" class=\"form-control\" >");
        try {
            ResultSetMetaData mdata = dat.getMetaData();
            int lim = mdata.getColumnCount();
            String et = "";
            ...
            while (dat.next()) {
                sel = "";
                if (dat.getString(1) != null) {
                    if (dat.getString(1).compareTo(mar) == 0 && marca) {
                        marca = false;
                        sel = "selected";
                    }
                }
    ...
    public static String comboRequired(ResultSet dat, String nom, String mar, String onC, String opt) {
        String sel = "";
        StringBuilder cad = new StringBuilder();
        cad.append("<select id=\"" + nom + "\" name=\"" + nom + "\" class=\"form-control\" onchange=\"" + onC + "\" required>");
        if (opt.compareTo("") != 0) {
            cad.append("<option value=''>" + opt + "</option>");
        }
        try {
            ResultSetMetaData mdata = dat.getMetaData();
            int lim = mdata.getColumnCount();
    ...
    public static String combo(ResultSet dat, String nom, String mar, String onC, String opt, int anch) {
        String sel = "";
        StringBuilder cad = new StringBuilder();
        cad.append("<select id=\"" + nom + "\" name=\"" + nom + "\" onchange=\"" + onC + "\" class=\"form-control\" style=\"width:" + anch + "px\">");
        if (opt.compareTo("") != 0) {
            cad.append("<option value='-0'>" + opt + "</option>");
        }
        try {
    ...
    public static String combo(ResultSet dat, String nom, String mar, String onC, String opt, int anch, String estado) {
        String sel = "";
        StringBuilder cad = new StringBuilder();
        cad.append("<select id=\"" + nom + "\" name=\"" + nom + "\" onchange=\"" + onC + "\" style=\"width:" + anch + "px\" " + estado + ">");
        if (opt.compareTo("") != 0) {
            cad.append("<option value='-0'>" + opt + "</option>");
        }
        try {
    ...
    public static String combo(ResultSet dat, String nom, String mar, String onFoc, String evt, String onCh, String opt, int anch) {
        String sel = "";
        StringBuilder cad = new StringBuilder();
        cad.append("<select id=\"" + nom + "\" name=\"" + nom + "\" style=\"width:" + anch + "px\" onfocus=\"" + onFoc + "\" onkeyup=\"" + evt + "\" onchange=\"" + onCh + "\" "
                + "onblur=\"_R('mSg')\">");
    ...
    /**
     * B�squeda binaria de una clave en una tabla de referencia. Se requiere que
     * los valores est�n ordenados.
     *
     ...
     * @return retorna el �ndice si la clave se encuantra en el vector caso
     * contrario -1.
     */
     ...
     * contrario -1.
     */
    public static int enMatrizSecuencial(String[][] matriz, String clave, int pos) {
     ...
     * contrario -1.
     */
    public static int enMatrizSecuencialSucursal(String[][] matriz, String clave, int pos, String sucursal) {
        for (int i = 0; i < matriz.length; i++) {
            if (matriz[i][0].compareTo(sucursal) == 0 && matriz[i][pos].compareTo(clave) == 0) {
                return i;
            }
        }
        return -1;
    ...
    /**
     * Funci�n que imprime los datos de una base de datos en formato de combobox
     * o lista en forma de �rbol seg�n sea definido el par�metro filas, por
     * defecto es 1 e indica que se mostrar� un combo, si es mayor a 1, entonces
     ...
     * @param onC. Funci�n javascript que se ejecuta cuando se ejecute el evento
     * onChange.
    ...
    /**
     * Funci�n que imprime los datos de una base de datos en formato de tabla
     * HTML.
     *
     * @param datos. Juego de registros de datos.
     ...
     * @param onC. Funci�n javascript que se ejecuta cuando se haga clic en el
     * registro.
